...
class SemanticCacheIndex:
    """
    Index for fast semantic cache lookups.

    Maintains inverted index of patterns → cache keys
    ...
    def __init__(self, redis_client):
        ...
        """
        self.redis = redis_client
        self.index_prefix = "semantic_index"
    ...
    async def add_to_index(
        ...
        """
        Add cache key to pattern index.

        Args:
            cache_key: Semantic cache key
            patterns: List of patterns
            ttl: Time to live for index entries
        """
        for pattern in patterns:
            index_key = f"{self.index_prefix}:{pattern}"
            ...
            # Set expiry
            await self.redis.expire(index_key, ttl + 60)

    async def find_by_patterns(
        self,
        patterns: List[str],
        min_match: int = 1
    ) -> List[str]:
        """
        Find cache keys that match at least N patterns.

        Args:
            patterns: Patterns to search for
            min_match: Minimum number of patterns to match
        ...
        for pattern in patterns:
            index_key = f"{self.index_prefix}:{pattern}"
            # Get all keys with this pattern
            keys = await self.redis.zrange(index_key, 0, -1)
            for key in keys:
                key_counts[key] += 1
    ...
    async def remove_from_index(self, cache_key: str) -> None:
        """
        Remove cache key from all pattern indexes.

        Args:
            cache_key: Cache key to remove
        """
        # Scan all pattern indexes
        async for key in self.redis.scan_iter(match=f"{self.index_prefix}:*"):
            await self.redis.zrem(key, cache_key)
