...
class TestAlertStateTracker:
    ...
    async def test_state_tracker_initialization(self, clean_state):
        """Test that AlertStateTracker initializes correctly."""
        tracker = AlertStateTracker()

        assert tracker is not None
        assert tracker.firing_count == 0
        assert isinstance(await tracker.get_all_state(), dict)

    async def test_set_firing_transitions_state(self, clean_state):
        """Test that set_firing transitions rule to firing state."""
        tracker = AlertStateTracker()

        state = await tracker.set_firing("test-rule-001")

        assert state["status"] == "firing"
        assert "fired_at" in state

        # Verify state is persisted
        retrieved = await tracker.get("test-rule-001")
        assert retrieved["status"] == "firing"

    async def test_set_resolved_transitions_state(self, clean_state):
        """Test that set_resolved transitions rule to resolved state."""
        tracker = AlertStateTracker()

        # First set to firing
        await tracker.set_firing("test-rule-001")

        # Then resolve
        state = await tracker.set_resolved("test-rule-001")

        assert state["status"] == "resolved"
        assert "resolved_at" in state

    async def test_get_state_returns_current_state(self, clean_state):
        """Test that get() returns current state for a rule."""
        tracker = AlertStateTracker()

        await tracker.set_firing("test-rule-001")

        state = await tracker.get("test-rule-001")

        assert state is not None
        assert state["status"] == "firing"

    async def test_get_state_returns_none_for_unknown_rule(self, clean_state):
        """Test that get() returns None for unknown rule."""
        tracker = AlertStateTracker()

        state = await tracker.get("unknown-rule")

        assert state is None

    async def test_state_includes_timestamp(self, clean_state):
    ...
    async def test_multiple_rules_independent_states(self, clean_state):
        """Test that multiple rules can have independent states."""
        tracker = AlertStateTracker()

        await tracker.set_firing("rule-001")
        await tracker.set_firing("rule-002")

        state1 = await tracker.get("rule-001")
        state2 = await tracker.get("rule-002")

        assert state1["status"] == "firing"
        assert state2["status"] == "firing"

    async def test_firing_count(self, clean_state):
        ...
        await tracker.set_resolved("rule-001")
        assert tracker.firing_count == 1

    async def test_all_state_returns_all_states(self, clean_state):
        ...
        all_states = await tracker.get_all_state()
...
class TestAlertHistory:
    ...
    async def test_history_initialization(self, clean_state):
    ...
    async def test_add_event_creates_history_entry(self, clean_state):
    ...
    async def test_add_event_includes_timestamp(self, clean_state):
    ...
    async def test_entries_returns_list(self, clean_state):
    ...
    async def test_max_entries_enforcement(self, clean_state):
        ...
        for i in range(5):
            await history.add({"rule_id": f"rule-{i}", "status": "firing"})

        # Should not exceed max_entries
        entries = history.entries
        assert len(entries) <= 3

    async def test_multiple_events_tracked(self, clean_state):
