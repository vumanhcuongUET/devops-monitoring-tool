...
class TestApprovalStateTracker:
    ...
    async def test_set_status(self, tracker):
        ...
        state = await tracker.get("act-123")
        assert state is not None
        assert state["status"] == ActionStatus.PENDING
        assert state["updated_by"] == "john.doe"
        assert "updated_at" in state

    async def test_set_status_with_command(self, tracker):
    ...
    async def test_set_status_overwrites_existing(self, tracker):
        """Test that set_status overwrites existing state."""
        await tracker.set_status(action_id="act-123", status=ActionStatus.PENDING)
        await tracker.set_status(
            action_id="act-123",
            status=ActionStatus.APPROVED,
            user="admin",
    ...
    async def test_get_action_exists(self, tracker):
        """Test getting existing action."""
        await tracker.set_status(action_id="act-123", status=ActionStatus.PENDING)

        result = await tracker.get("act-123")

        assert result is not None
        assert result["status"] == ActionStatus.PENDING

    async def test_get_action_not_exists(self, tracker):
        """Test getting non-existent action."""
        result = await tracker.get("nonexistent")

        assert result is None

    async def test_get_all(self, tracker):
        """Test getting all actions."""
        await tracker.set_status(action_id="act-1", status=ActionStatus.PENDING)
        await tracker.set_status(action_id="act-2", status=ActionStatus.APPROVED)
        await tracker.set_status(action_id="act-3", status=ActionStatus.REJECTED)

        all_state = await tracker.get_all()
    ...
    async def test_get_all_empty(self, clean_approval_state):
    ...
    async def test_get_by_status(self, tracker):
        """Test getting actions by status."""
        await tracker.set_status(action_id="act-1", status=ActionStatus.PENDING)
        await tracker.set_status(action_id="act-2", status=ActionStatus.PENDING)
        await tracker.set_status(action_id="act-3", status=ActionStatus.APPROVED)

        pending = await tracker.get_by_status(ActionStatus.PENDING)
        approved = await tracker.get_by_status(ActionStatus.APPROVED)
    ...
    async def test_get_by_status_empty(self, tracker):
    ...
    async def test_get_pending_count(self, tracker):
        ...
        count = await tracker.get_pending_count()

        assert count == 2

    async def test_get_pending_count_zero(self, clean_approval_state):
        """Test getting pending count when none pending."""
        tracker = ApprovalStateTracker()
        await tracker.set_status(action_id="act-1", status=ActionStatus.APPROVED)

        count = await tracker.get_pending_count()

        assert count == 0

    async def test_delete_action_exists(self, tracker):
        """Test deleting existing action."""
        await tracker.set_status(action_id="act-123", status=ActionStatus.PENDING)

        result = await tracker.delete("act-123")

        assert result is True
        assert await tracker.get("act-123") is None

    async def test_delete_action_not_exists(self, tracker):
        """Test deleting non-existent action."""
        result = await tracker.delete("nonexistent")

        assert result is False

    async def test_set_ws_manager(self, tracker, mock_ws_manager):
...
class TestApprovalHistory:
    ...
    async def test_add_history_entry(self, history):
    ...
    async def test_get_history_for_action(self, history):
    ...
    async def test_get_history_empty(self, history):
        """Test getting history for action with no history."""
        history_list = history.get_for_action("nonexistent")

        assert len(history_list) == 0

    async def test_get_all_history(self, history):
        """Test getting all history."""
        await history.add({"id": "1", "action_id": "act-1", "event": "created"})
        await history.add({"id": "2", "action_id": "act-2", "event": "created"})

        all_history = history.entries

        assert len(all_history) >= 2

    async def test_history_max_entries(self, clean_approval_state):
...
class TestApprovalTrackerSingleton:
    ...
    async def test_get_approval_tracker_returns_singleton(self):
    ...
    async def test_get_approval_tracker_initializes_new_instance(self):
...
class TestApprovalStateTrackerErrorHandling:
    ...
    async def test_loading_corrupted_json_file(self, clean_approval_state):
        ...
        assert await new_tracker.get("nonexistent") is None
        assert len(await new_tracker.get_all()) == 0

    async def test_saving_with_permission_error(self, tracker, clean_approval_state, monkeypatch):
